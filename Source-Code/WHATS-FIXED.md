...
## Round 36 — six more "Apply DDL to Target" gaps in the same package, plus a native-partitioning preflight check (build R36, 17 Sep 2026)

Round 35 fixed one failure ("unrecognized exception condition") out of a
batch of eight that all showed up in the same real migration log, on the
same PL/SQL package. This round fixes the remaining six, plus a
completely separate, newly-reported failure class in the partitioned
tables underneath the same schema.

**1. Oracle collection/array element assignment (`v_values(1):=100;`) --
now caught, not silently mismapped.** A variable holding an Oracle
collection whose type Postgres has no equivalent for isn't always
declared where this tool can see it (a package-specification-only type,
or an Oracle built-in collection type used directly) -- so it used to
fall through to a generic scalar type mapping with no warning at all,
and `v_values(1):=100;` reached "Apply DDL to Target" as live, broken
SQL: `syntax error at or near "v_values"`. This is now detected directly
from the assignment syntax itself (`name(args) := value` is invalid
PL/pgSQL no matter how, or whether, the variable's type could be
resolved) and flagged "Requires manual conversion" before it ever
reaches the target.

**2. A parameterized CURSOR's own parameter types are now mapped.**
`CURSOR c(cp_category VARCHAR2) IS SELECT ...` was carrying its
parameter list through completely untranslated -- unlike a routine's own
parameters, which were already mapped -- so it reached the target as
literal `CURSOR c(cp_category VARCHAR2) FOR SELECT ...`, and Postgres
rejected it outright: `type "varchar2" does not exist`. Cursor
parameters now go through the exact same type mapping a routine's own
parameters do.

**3. A bare procedure-call statement now gets `CALL`.** Oracle allows an
unqualified procedure call as a standalone statement with no leading
keyword (`feature_pl_create_row(p_id, p_name);`) -- valid there because
PL/SQL has no way to call a *function* and discard its result other than
assigning it, so a bare statement can only ever be a procedure call.
PL/pgSQL has no such bare-call form at all; every statement must start
with a keyword its parser recognizes, so this failed with
`syntax error at or near "feature_pl_create_row"`. This is now detected
(a real standalone call, not one used inside an assignment or an IF
condition, which must not get a CALL prefix) and rewritten with the
`CALL` keyword Postgres requires.

**4. `SYS_REFCURSOR` now maps to PostgreSQL's `REFCURSOR`.** Oracle's
predefined weak ref cursor type had no mapping rule at all and silently
became `TEXT`. A routine that then did `OPEN p_cursor FOR SELECT ...;`
against that `TEXT` parameter failed with
`variable "p_cursor" must be of type cursor or refcursor` -- with no
warning ever raised during conversion to explain why. `SYS_REFCURSOR`
now maps directly to `REFCURSOR`, PostgreSQL's own equivalent.

**5. `PIPE ROW` is now flagged manual, not left to fail at apply time.**
Oracle's pipelined-table-function idiom (`PIPE ROW(...)` inside a
`FUNCTION ... PIPELINED`) streams rows to its caller before the function
has finished running -- PostgreSQL has no equivalent statement at all; a
set-returning function needs a completely different shape (`RETURNS
SETOF`/`RETURNS TABLE` with `RETURN NEXT`/`RETURN QUERY`), which can't be
safely retrofitted onto an arbitrary existing function body. Left as-is
this failed with `syntax error at or near "PIPE"`. It's now flagged
"Requires manual conversion" up front instead.

**6. `ROLLBACK TO` outside an exception handler: confirmed already
correct, not a new bug.** This one was already handled by an earlier
round: `ROLLBACK TO` inside an exception handler is dropped (redundant
with Postgres's automatic rollback-on-catch), but the same statement
appearing in the main executable flow has no safe automatic translation
and is deliberately left as-is and flagged "Requires manual conversion"
instead of guessed at. If this is still reaching a live target, the fix
is to exclude that routine from the DDL batch (or rewrite it by hand as
a nested `BEGIN ... EXCEPTION ... END` block) -- not a new code change.

**7. New: a partitioned table already on the target as a plain table is
now caught before "Apply DDL to Target" runs a single statement.**
Reported separately, from the same schema: 34 consecutive statement
failures, all reading `"orders" is not partitioned` (and the same for
`adv_feature_partition`), with `Apply DDL to Target` otherwise reporting
success. Root cause: `CREATE TABLE IF NOT EXISTS orders (...) PARTITION
BY RANGE (...)` against a target that already has a plain (unpartitioned)
`orders` table -- left over from before this tool supported native
partitioning, or from an earlier run where the table happened to fall
back to unpartitioned -- silently does nothing, since PostgreSQL has no
`ALTER TABLE` that turns an existing plain table into a partitioned one.
Every `CREATE TABLE ... PARTITION OF orders ...` statement that follows
then fails, one per partition, with an error that never names the actual
cause. The existing "Apply DDL to Target" preflight check (added for the
analogous column-shape mismatch, see Round 28) now also asks this second
question for a partitioned source table, and reports it through the same
"Leave it out" / "Replace it..." recovery dialog -- no new UI, since the
existing one already does exactly what's needed here: drop the stale
table and re-apply.

**What this means for the migration that hit these errors.** Re-running
"2. Convert Schema" then "3. Apply DDL to Target" now gets past all
seven of the above. `v_values`, `ROLLBACK TO` (outside a handler), and
`PIPE ROW` still require a manual rewrite -- that's the correct outcome,
not a remaining bug -- but they're now clearly flagged instead of
reaching the target as broken SQL. The two partitioned tables will need
either "Replace it..." (drops and recreates them, correctly partitioned)
or a manual `DROP TABLE ... CASCADE` on the target first.
